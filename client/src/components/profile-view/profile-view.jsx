...
export class ProfileView extends React.Component {
    ...
    deleteFavouriteMovie(movieId) {
        ...
        axios.delete(`https://myflixmovies.herokuapp.com/users/${username}/Movies/${movieId}`, {
            headers: { Authorization: `Bearer ${token}` }
        })
            .then(response => {
    ...
    render() {
        const { username, email, password, birthday, favouriteMovies, userData } = this.state;
        const favouriteMovieList = this.props.movies.filter(m => this.state.favouriteMovies.includes(m._id));
                            ...
                            <Col>
                                ...
                                <Form>
                                    ...
                                    <Form.Group>
                                        <Form.Label>
                                            Password:
                                            <Form.Control type="password" plaintext readOnly defaultValue="******" />
                                        </Form.Label>
                                    </Form.Group>
                                    <Form.Group>
                                        <Form.Label>
                                            Email address:
                                            <Form.Control type="email" plaintext readOnly defaultValue={email} />
                                        </Form.Label>
                                    </Form.Group>
                                    <Form.Group>
                                        <Form.Label>
                                            Birthday:
                                            <Form.Control type="string" plaintext readOnly defaultValue={birthday && birthday.slice(0, 10)} />
                                        </Form.Label>
                                    </Form.Group>
                                    <Button className="edit-button" href="/editprofile" >Edit Profile</Button>
                                </Form >
                            </Col>
                        </Row>
                        <Row>
                                        ...
                                        <CardDeck className="card-deck">
                                            {favouriteMovieList.map(m => (
                                                <div className="favourite-movie">
                                                    ...
                                                    <Button className="remove-favourite-button" size='sm' onClick={e => this.deleteFavouriteMovie(m._id)}>Remove from favourites</Button>
                                                </div>
